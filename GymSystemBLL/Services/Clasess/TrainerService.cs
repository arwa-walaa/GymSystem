    ...
    internal class TrainerService : Interfaces.ITrainerService
    {
        ...
        public IEnumerable<TrainerViewModel> GetAllTrainers()
        {
            ...
            return Trainers.Select(X => new TrainerViewModel()
            {
                ...
                Email = X.Email,
                Phone = X.Phone,
                Specialites = X.Specialites.ToString()
            });
        }
        ...
        {
            return _unitOfWork.GetRepo<Trainer>().GetAll(X => X.Email == email).Any();
        }
        private bool IsPhoneExist(string phone)
        {
            return _unitOfWork.GetRepo<Trainer>().GetAll(X => X.Phone == phone).Any();
        }
        private bool HasActiveSessions(int id)
        {
            var activeSessions = _unitOfWork.GetRepo<Session>()
                .GetAll(S => S.TrainerId == id && S.StratDate > DateTime.Now).Any();
